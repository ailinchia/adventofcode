...
for line in sys.stdin:
    ...
    while i < len(items):
        ...
        if opcode == 1:  # adds
            ...
            if str(point_robot) in panels:
                (point, input_signal) = panels[str(point_robot)]
            ...
            if j % 2 == 0:
                ...
                panels[str(point_robot)] = (Point(point_robot.x, point_robot.y), color)
            else:
                ...
                if max_x < point_robot.x:
                    max_x = point_robot.x
                if max_y < point_robot.y:
                    max_y = point_robot.y
            j += 1
            step = 2
    ...
    side = [[0 for i in range(max_x + 1)] for j in range(max_y + 1)]
    for key in panels:
        point, color = panels[key]
        side[point.y][point.x] = color
